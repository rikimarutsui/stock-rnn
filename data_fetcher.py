...
def _download_sp500_list():
    ...
    f = http.request('GET', SP500_LIST_URL)
    #f = urllib3.urlopen(SP500_LIST_URL)
...
def fetch_prices(symbol, out_name):
    ...
    symbol_url = BASE_URL.format(
        urllib3.quote(symbol),
        urllib3.quote(now_datetime, '+')
    )
    ...
    try:
        http = urllib3.PoolManager()
        f = http.request('GET', symbol_url)#f = urllib3.urlopen(symbol_url)
